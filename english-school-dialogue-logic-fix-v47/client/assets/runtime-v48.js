    ...
    card.querySelectorAll('button').forEach(button=>{
      button.addEventListener('click',()=>{
        if(button.textContent.trim()==='4'){
          ...
          setTimeout(()=>{card.remove();wordCard.removeAttribute('data-count-locked');},450);
        }else{
    ...
    if(playButton&&!playButton.dataset.roySongBound){
      ...
      playButton.addEventListener('click',()=>{
        ...
        setTimeout(()=>{
          ...
          if(audio){audio.addEventListener('ended',finish,{once:true});audio.addEventListener('error',finish,{once:true});}
        },250);
